<template>
  <svg xmlns="http://www.w3.org/2000/svg" :color="color" :role="role" :aria-labelledby="icon" focusable="false"
    aria-hidden="true" class="c-btn__icon">
    <title :id="name || icon">{{ name || icon }} Icon</title>
    <desc>{{ iconDescription || name }}</desc>
    <svg width="16" height="16" viewBox="0 0 16 16" v-html="paths[icon]" />
  </svg>
</template>
